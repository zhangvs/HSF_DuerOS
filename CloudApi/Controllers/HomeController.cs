...
namespace CloudApi.Controllers
{
    ...
    { 
        ...
        public async Task<ActionResult> Index2()
        {
            ...
            //判断登录验证情况
            string host = CheckLogin(account, password);
            if (host != null)
            {
                string hostId = host;//要授权的网关ID或设备ID
                string sessionID = "";
                return Redirect("~/authorize?grant_type=authorization_code&response_type=code&client_id=" + clientId + "&account=" + account + "&pwd=" + password + "&sessionId=" + sessionID + "&hostId=" + hostId + "&redirect_uri=" + urlEncode);
            }
            else
            {
        ...
        public async Task<ActionResult> TestToken()
        {
            ...
            await Task.Run(() => {
                if (grant_type.Equals("authorization_code", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    //{
                    //    service.Publish("LotDeviceChangeQueue", hostId);
                    //}
                }
